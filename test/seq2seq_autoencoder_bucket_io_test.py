...
#         while line:
#             if line == data_utils.file_termimal:
#                 line = read_file.readline()
#                 continue
#             else:
#                 line_ids = [int(x) for x in line.rstrip().split()]
#                 if forward_data_feed:
#                     source_ids = line_ids
#                     target_ids = list(line_ids) + [data_utils.EOS_ID]
...
def read_data(path, data, forward_data_feed):
    ...
    with open(path, 'r') as read_file:
        ...
        while third_line:
            ...
            for bucket_id, seq_size in enumerate(buckets):
                if len(source_ids) < seq_size and len(target_ids) < seq_size:
                    data[bucket_id].append([source_ids, target_ids])
                    break
